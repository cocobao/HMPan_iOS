...
-(UIView*)backView
{
    if (!_backView) {
        ...
        [_backView setBackgroundColor:Color_black(30)];
...
-(UITableView*)mTableView
{
    if (!_mTableView) {
        ...
        _mTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, kScreenHeight, kScreenWidth, (cellHeigh*_mArrData.count)) style:UITableViewStylePlain];
...
-(void)showModifyNamePageView
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        weakSelf.mTableView.frame = CGRectMake(0, kScreenHeight-(self.mTableView.rowHeight * _mArrData.count), kScreenWidth, self.frame.size.height);
    }];
}
...
-(void)removeMySelf
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        _backView.alpha = 0;
        weakSelf.mTableView.frame = CGRectMake(0, kScreenHeight, kScreenWidth, self.frame.size.height);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        ...
        UIView *selectView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, tableView.rowHeight)];
        selectView.backgroundColor = Color_black(5);
        cell.selectedBackgroundView = selectView;
    }
    if (indexPath.row != (self.mArrData.count-1)) {
        CALayer *line = [CALayer layer];
        line.frame = CGRectMake(MarginW(20), tableView.rowHeight-0.5, kScreenWidth-MarginW(40), 0.5f);
    ...
    if ([obj isKindOfClass:[UIImage class]]) {
        ...
        imageView.center = CGPointMake(kScreenWidth/2, tableView.rowHeight/2);
        ...
        if (indexPath.row == _mArrData.count - 1 && _lastColor) {
            ...
            [cell.textLabel setTextColor:Color_black(50)];
        }
