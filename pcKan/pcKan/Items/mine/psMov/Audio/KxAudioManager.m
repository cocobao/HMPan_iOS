...
- (BOOL) setupAudio
{
    ...
    //监听RouteChange事件, 如果想要实现类似于“拔掉耳机就把歌曲暂停”的功能就需要监听RouteChange事件
...
- (BOOL) activateAudioSession
{
    if (!_activated) {
        ...
        if (!_initialized) {
            ...
            //初始化AudioSession
            //前两个参数一般填NULL表示AudioSession运行在主线程上（但并不代表音频的相关处理运行在主线程上，只是AudioSession）
            //第三个参数需要传入一个一个AudioSessionInterruptionListener类型的方法，作为AudioSession被打断时的回调
            //第四个参数则是代表打断回调时需要附带的对象
...
//AudioSessionInitialize可以被多次执行，但AudioSessionInterruptionListener只能被设置一次，这就意味着这个打断回调方法是一个静态方法，一旦初始化成功以后所有的打断都会回调到这个方法，即便下一次再次调用AudioSessionInitialize并且把另一个静态方法作为参数传入，当打断到来时还是会回调到第一次设置的方法上。
static void sessionInterruptionListener(void *inClientData, UInt32 inInterruption)
{    
    KxAudioManagerImpl *sm = (__bridge KxAudioManagerImpl *)inClientData;
