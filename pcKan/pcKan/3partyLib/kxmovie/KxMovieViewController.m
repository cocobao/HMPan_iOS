...
- (void) playDidTouch: (UIButton *) sender
{
    if (self.playing){
        sender.selected = YES;
        [self pause];
...
-(void)fullAction:(UIButton *)sender
{
    if (sender.selected) {
        ...
        [UIView animateWithDuration:0.3f animations:^{
            ...
            _glView.center = CGPointMake(kScreenWidth/2, (kScreenHeight)/2-NAVBAR_H);
        ...
        [UIView animateWithDuration:0.3f animations:^{
            ...
            [_glView setFrame:CGRectMake(0, 0, width, height)];
            self.ctrlView.frame = CGRectMake(0, frame.size.height-40, frame.size.width, 40);
...
{
    CGRect bounds = CGRectMake(0, 0, kScreenWidth, kScreenWidth*9/16+NAVBAR_H);
    ...
    if (!_glView) {
        ...
        self.ctrlView.frame = CGRectMake(0, bounds.size.height-40, bounds.size.width, 40);
        [_glView addSubview:self.ctrlView];
    }
...
-(pssMediaControlView *)ctrlView
{
    if (!_ctrlView) {
        ...
        [view.playerBtn addTarget:self action:@selector(playDidTouch:) forControlEvents:UIControlEventTouchUpInside];
...
//        [view.mediaProgressSlider addTarget:self action:@selector(didSliderTouchCancel) forControlEvents:UIControlEventTouchCancel];
        [view.mediaProgressSlider addTarget:self action:@selector(progressDidChange:) forControlEvents:UIControlEventValueChanged];
...
- (void) updateHUD
{
    ...
    if (_ctrlView.mediaProgressSlider.state == UIControlStateNormal)
        _ctrlView.mediaProgressSlider.value = position / duration;
    
//    if (_progressSlider.state == UIControlStateNormal) {
//        _progressSlider.value = position / duration;
//    }
    
    _ctrlView.currentTimeLabel.text = formatTimeInterval(position, NO);
    
    if (_decoder.duration != MAXFLOAT)
        _ctrlView.totalDurationLabel.text = formatTimeInterval(duration - position, YES);
